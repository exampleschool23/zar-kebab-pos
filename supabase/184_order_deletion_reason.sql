...
alter table public.order_change_investor_notification_deliveries
  add column deletion_reason text;
-- Set the default separately so historical notifications remain unchanged.
alter table public.order_change_investor_notification_deliveries
  alter column deletion_reason set default nullif(current_setting('app.order_deletion_reason', true), '');

drop function public.delete_order_owner(text);

create or replace function public.delete_order_owner(p_order_id text, p_reason text default null)
returns jsonb
language plpgsql
security definer
...
begin
  if nullif(btrim(p_reason), '') is null or length(btrim(p_reason)) > 1000 then
    raise exception 'A deletion reason of 1–1000 characters is required' using errcode = '22023';
  end if;
  perform set_config('app.order_deletion_reason', btrim(p_reason), true);
  if not public.current_staff_can_access('delete_paid_orders') then
    raise exception 'Delete paid orders access is required' using errcode = '42501';
  end if;
...
revoke all on function public.delete_order_owner(text, text) from public;
grant execute on function public.delete_order_owner(text, text) to authenticated;
grant execute on function public.delete_order_owner(text, text) to service_role;
