-- Bounded ten-day dashboard aggregate: at most one month, no order/item payloads.
begin;
...
create or replace function public.get_dashboard_weekly_average_income(p_month_start date)
...
declare
  ...
  v_from_instant timestamptz := p_month_start::timestamp at time zone 'Asia/Tashkent';
...
begin
  ...
  with paid_orders as materialized (
    ...
    select p_month_start + offset_days as start_date,
      case when offset_days = 20 then v_end else p_month_start + offset_days + 10 end as end_date
    from generate_series(0, 20, 10) offset_days
  )
...
$$;
revoke all on function public.get_dashboard_weekly_average_income(date) from public, anon, authenticated;
grant execute on function public.get_dashboard_weekly_average_income(date) to authenticated;
