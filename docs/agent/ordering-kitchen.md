...
- Mobile staff ordering defaults to a compact product grid below 640px (three columns from 420px, two on narrower phones), with a locally saved compact/large card switch. Mobile categories use one sticky chip row. Guest tablet mode keeps cards. Both views share price, variant, and cart handlers; details retain descriptions and nutrition.
