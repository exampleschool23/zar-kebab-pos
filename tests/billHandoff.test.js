...
test('blocked fresh submission preserves cart and does not persist a retry; pending retries bypass preflight', async () => {
  ...
  const preflight = new Function('stateRef', 'pendingKitchenSubmissionRef', 'action', 'dispatch', 'normalizeOrderType', 'isOffPremiseOrderType', 'getKitchenBillBlockError', `let enriched; let reusedPendingKitchenSubmission; ${source.slice(start, end)} } return { enriched, reusedPendingKitchenSubmission };`)
  const { getKitchenBillBlockError } = await import('../src/lib/billHandoff.js')
  const state = { orders: [{ id: 'o1', table_id: 't1', status: 'needs_bill', payment_status: 'unpaid' }], currentTableId: 't1', user: { role: 'admin', feature_access: ['tables'] }, cart: [{ id: 'dish1', quantity: 2 }] }
  const original = structuredClone(state)
  const notices = []
  const run = pending => preflight({ current: state }, { current: pending }, { type: 'SEND_TO_KITCHEN', payload: { orderType: 'dine_in' } }, action => notices.push(action), value => value, type => type !== 'dine_in', getKitchenBillBlockError)
