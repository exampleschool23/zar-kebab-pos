...
test('Auto-print bill saves immediately and prints after a successful move to cashier', () => {
  ...
  assert.match(billHandoff, /const printQuery = autoPrint \? '\?print=1' : ''[\s\S]*navigate\(`\/cashier\/bill\/\$\{encodedTableId\}\$\{printQuery\}`\)/)
  ...
  assert.doesNotMatch(billHandoff, /completeBillPrint|printWindow|browserWindow/)
  assert.match(waiterTables, /MARK_TABLE_NEEDS_BILL[\s\S]*completeBillHandoff\(\{ navigate, tableId: table\.id, autoPrint \}\)/)
  assert.match(waiterOrder, /MARK_TABLE_NEEDS_BILL[\s\S]*onMovedToCashier\?\.\(\)/)
  assert.doesNotMatch(billHandoff, /window\.open|_blank|prepareBillPrintWindow/)
  ...
  assert.match(cashierBill, /async function handlePrintBill\(\)[\s\S]*refreshCurrentBill\(\)[\s\S]*getFreshCashierPaymentQuote\([\s\S]*setPrintRequest\(\{[\s\S]*getBillReceiptPath\(\{ tableId, orderId \}\)/)
  assert.match(cashierBill, /<iframe[\s\S]*src=\{printRequest\.path\}/)
  assert.doesNotMatch(cashierBill, /prepareBillPrintWindow|completeBillPrint|cancelBillPrintWindow/)
  assert.match(cashierBill, /import \{ getBillReceiptPath \} from '\.\.\/lib\/billHandoff'/)
...
test('paid order deletion is routed through feature-gated RPC and wired to cashier and reports', () => {
  ...
  assert.match(permissions, /function canDeleteOrderToday[\s\S]*date === formatDateOnly\(now\) && canDeletePaidOrders\(profileOrRole\)/)
  ...
  assert.doesNotMatch(dashboard, /RecentOrderRow|type: 'DELETE_ORDER'/)
  assert.match(cashier, /canDeleteOrderToday/)
  assert.match(cashier, /confirmDeleteOrder/)
  assert.match(cashier, /type: 'DELETE_ORDER'/)
  assert.match(cashierTables, /useAuth/)
  assert.match(cashierTables, /canDeleteOrderToday/)
  ...
  assert.match(reports, /canDeleteOrderToday/)
