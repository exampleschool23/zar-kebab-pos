...
test('completed order group message escapes dynamic Telegram HTML fields', () => {
  const message = buildCompletedOrderGroupMessage({
    ...
    items: [
      { name: 'Beef <Shashlik>', quantity: 1, price: 30000, status: 'served' },
  ...
  assert.match(message, /Beef &lt;Shashlik&gt;/)
  ...
  assert.match(message, /<b>К оплате: 163 300 UZS<\/b>/)
  assert.match(message, /Выручка сегодня: 525 300 UZS/)
})
