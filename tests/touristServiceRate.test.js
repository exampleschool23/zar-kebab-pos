...
test('all operational and historical fallbacks use the price-mode service setting', () => {
  ...
  const cashierCheckout = read('src/lib/cashierCheckout.js')
  ...
  assert.match(cashier, /getFreshCashierPaymentQuote\(\{/)
  assert.match(cashier, /cashierQuote\.priceMode \|\| normalizePriceMode\(primaryOrder\.price_mode\)/)
  assert.match(receipt, /getOrderGroupPaymentQuote\(\{ orders: allOrders, menuItems: state\.menuItems, settings \}\)/)
  assert.match(cashierCheckout, /getConfiguredServiceRatePct\(settings, order\.price_mode\)/)
