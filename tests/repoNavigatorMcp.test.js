...
test('indexes complete large components, PL/pgSQL bodies, and multiline protected routes', async () => {
  ...
  assert.equal(menuOutline.rows.find((row) => row[4] === 'ImageUploadField')?.[3], 'L322-374')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'OrangeBtn')?.[3], 'L376-388')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'VisibilityToggleButton')?.[3], 'L507-529')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'SortableItemRow')?.[3], 'L668-751')
