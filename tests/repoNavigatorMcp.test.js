...
test('indexes complete large components, PL/pgSQL bodies, and multiline protected routes', async () => {
  ...
  assert.equal(menuOutline.rows.find((row) => row[4] === 'ImageUploadField')?.[3], 'L409-474')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'MediaGalleryField')?.[3], 'L476-634')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'OrangeBtn')?.[3], 'L636-648')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'VisibilityToggleButton')?.[3], 'L819-841')
  assert.equal(menuOutline.rows.find((row) => row[4] === 'SortableItemRow')?.[3], 'L992-1089')
