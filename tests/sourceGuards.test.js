...
test('cashier payment waits for database success and supports legacy loyalty transaction constraints', () => {
  ...
  assert.match(db, /assertUpdatedRows\(paidRows[\s\S]*notifyTelegramOrderStatus\(finalSummaries\.map\(order => order\.id\), 'completed'\)/)
  assert.doesNotMatch(db, /await notifyTelegramOrderStatus\(finalSummaries/)
...
test('expenses page is feature-gated, persisted, and included in owner reports net income', () => {
  ...
  assert.match(expenseLib, /return getDailySalaryAmount\(salaryProfile, asOfDate\) \* 30/)
  ...
  assert.match(monthlyEstimate, /getOrderPayments/)
...
test('all accounting summary includes paid cafe income', () => {
  ...
  assert.match(history, /getCafeIncomeForRange\(state\.orders, dateFrom, dateTo\)/)
  assert.match(history, /\[state\.orders, dateFrom, dateTo\]/)
  ...
  assert.match(history, /totalExpenses: summarizeExpenses\(rows\.filter\(row => row\.expense_date === group\.date\)\)\.total/)
