...
test('menu items support required option variants with parent product ids', () => {
  ...
  assert.match(waiterOrder, /if \(menuItemRequiresOptions\(item\) \|\| isMenuItemSoldByWeight\(item\)\) \{\s*openDetail\(item\)/)
  assert.match(functionBody(waiterOrder, 'handleDecrement'), /if \(menuItemRequiresOptions\(item\) \|\| isMenuItemSoldByWeight\(item\)\) \{\s*openDetail\(item\)[\s\S]*return/)
  assert.match(kitchenCheckReceipt, /getManualOrderNotes\(item, menuItem, lang\)/)
