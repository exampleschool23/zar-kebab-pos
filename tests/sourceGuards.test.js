...
test('Kitchen can cancel unavailable items without billing them', () => {
  ...
  assert.match(kitchen, /handleMark\('cancelled', \{ reason, markMenuUnavailable \}\)/)
  assert.match(kitchen, /unavailableLabel\(lang\)/)
  assert.match(kitchen, /cancelReason/)
  assert.match(kitchen, /markMenuUnavailable/)
  ...
  assert.match(db, /markMenuUnavailable[\s\S]*menu_items[\s\S]*available: false/)
