...
test('expenses page is routed, protected, persisted, and included in reports net income', () => {
  const app = readSource('src/App.jsx')
  const sidebar = readSource('src/components/UnifiedSidebar.jsx')
  const permissions = readSource('src/lib/permissions.js')
  ...
  assert.match(permissions, /expenses:\s+\['owner', 'admin', 'cashier', 'stakeholder'\]/)
  ...
  assert.match(migration, /staff_read_expenses/)
  assert.match(migration, /accounting_staff_insert_expenses/)
  assert.match(migration, /owner_admin_delete_expenses/)
