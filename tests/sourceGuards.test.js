...
test('temporarily unavailable meals stay visible but cannot be ordered', () => {
  const menuItems = readSource('src/lib/menuItems.js')
  const publicMenu = readSource('src/pages/PublicMenu.jsx')
  const telegram = readSource('src/pages/TelegramMiniApp.jsx')
  const productCards = readSource('src/components/MenuProductCards.jsx')
  ...
  assert.doesNotMatch(functionBody(menuItems, 'isWaiterMenuItem'), /item\?\.available/)
  ...
  assert.match(productCards, /const unavailable = item\?\.available === false/)
  ...
  assert.match(cartPanel, /disabled=\{cart\.length === 0 \|\| isSending \|\| hasUnavailableCartItems/)
  assert.match(telegram, /unavailable: 'Unavailable'/)
...
test('tablet Guest mode locks staff navigation and never sends the guest cart to the kitchen', () => {
  const app = readSource('src/App.jsx')
  const waiterTables = readSource('src/pages/WaiterTables.jsx')
  const waiterOrder = readSource('src/pages/WaiterOrder.jsx')
  const cartPanel = readSource('src/components/CartPanel.jsx')
  const guestMode = readSource('src/lib/guestMode.js')
  const guestCart = readSource('src/lib/guestCart.js')
  const entryDialog = readSource('src/components/GuestModeUI.jsx')
  const routeLock = app.slice(app.indexOf('function GuestModeRouteLock'), app.indexOf('// Decides where to send user after login'))

  assert.match(app, /function GuestModeRouteLock/)
  assert.match(routeLock, /lockState === 'recovery'/)
  assert.match(routeLock, /lockState === 'signed_out'/)
  assert.match(routeLock, /ownsGuestModeSession: isGuestModeOwnedByCurrentTab/)
  assert.match(routeLock, /<GuestModeRecoveryLock guestModeSession=\{guestModeSession\} \/>/)
  assert.doesNotMatch(routeLock, /clearGuestModeSession\(\)/)
  assert.match(app, /location\.pathname !== guestModeSession\.routePath/)
  assert.match(app, /<Navigate to=\{guestModeSession\.routePath\} replace \/>/)
  assert.match(waiterOrder, /const isGuestTabletMode\s+= isGuestModeSessionFor/)
  assert.match(waiterOrder, /const shouldShowSidebar\s+= role !== 'guest' && !isGuestTabletMode/)
  assert.match(waiterOrder, /!isGuestTabletMode && !isTakeAwayFlow && \(/)
  assert.match(waiterOrder, /!isGuestTabletMode && \([\s\S]*?<OrderActionPanel/)
  assert.match(waiterOrder, /allowOrderTypeChange=\{!isGuestTabletMode\}/)
  assert.match(waiterOrder, /guestMode=\{isGuestTabletMode\}/)
  assert.match(waiterOrder, /onGuestFinish=\{finishGuestSelection\}/)
  assert.match(waiterTables, /<TableGuestEntryDialog/)
  assert.match(waiterTables, /const session = await createGuestModeSession\(/)
  assert.match(waiterTables, /writeGuestModeSession\(session\)[\s\S]*navigate\(session\.routePath, \{ replace: true \}\)/)
  assert.match(waiterTables, /priceMode: selectedPriceMode/)
  assert.doesNotMatch(waiterOrder, /openGuestModeSetup|activateGuestMode|handToGuest/)
  assert.match(waiterOrder, /if \(preserveUnlockedGuestPriceModeRef\.current\) \{\s*setPriceMode\(preserveUnlockedGuestPriceModeRef\.current\)/)
  assert.match(waiterOrder, /const guestSessionPriceMode = normalizeGuestModePriceMode\(guestModeSession\?\.priceMode\)/)
  assert.match(waiterOrder, /const restoredCart = rebuildGuestCartFromCatalog/)
  assert.match(waiterOrder, /const reviewedCart = rebuildGuestCartFromCatalog/)
  assert.match(waiterOrder, /cart: guestCartReady \? state\.cart : \(latestSession\.cart \|\| \[\]\)/)
  assert.match(waiterOrder, /preserveUnlockedGuestPriceModeRef\.current = normalizeGuestModePriceMode\(latestSession\.priceMode\)[\s\S]*clearGuestModeSession\(\)/)
  assert.match(cartPanel, /onClick=\{guestMode \? onGuestFinish : handleSend\}/)
  assert.match(cartPanel, /guestMode[\s\S]*?'Finish selection'/)
  assert.match(cartPanel, /A waiter will review and send this selection to the kitchen\./)
  assert.match(guestMode, /localStorage/)
  assert.match(guestMode, /sessionStorage/)
  assert.match(guestMode, /ownerTabId/)
  assert.match(guestMode, /name: 'PBKDF2'/)
  assert.match(guestMode, /getGuestModeLockState/)
  assert.doesNotMatch(guestMode, /pin:\s*normalizedPin/)
  assert.match(guestCart, /isCustomerMenuItem\(item, now\)/)
  assert.match(guestCart, /isCustomerMenuCategory\(category, now\)/)
  assert.match(guestCart, /calculateUnitPrice\(basePrice, priceMode\)/)
  assert.match(guestMode, /priceMode === PRICE_MODE_REGULAR \|\| priceMode === PRICE_MODE_TOURIST/)
  assert.match(entryDialog, /export function TableGuestEntryDialog/)
  assert.match(entryDialog, /type="radio"[\s\S]*name="guest-table-price-mode"/)
  assert.match(entryDialog, /shortLabel: 'R'/)
  assert.match(entryDialog, /shortLabel: 'T'/)
  assert.match(entryDialog, /aria-label=\{choice\.label\}/)
  assert.doesNotMatch(entryDialog, /regularPriceBody|touristPriceBody/)
  assert.match(waiterTables, /activeOrderIds: latestContext\.activeOrderIds/)
  assert.match(waiterOrder, /reviewWarning=\{guestReviewWarning\}/)
  assert.match(waiterOrder, /setPriceMode\(resolveOrderingPriceMode\(activeOrder, state\.cart\)\)/)
  assert.doesNotMatch(waiterOrder, /guestModeCopy\(lang\)\.touristMenu/)
})

test('tablet Guest PIN fields accept digits only and request a numeric keyboard', () => {
  const source = readSource('src/components/GuestModeUI.jsx')
  const guestMode = readSource('src/lib/guestMode.js')
  const guestPinDialog = source.slice(
    source.indexOf('export function GuestPinDialog'),
    source.indexOf('export function TableGuestEntryDialog')
  )
  const tableEntryDialog = source.slice(source.indexOf('export function TableGuestEntryDialog'))

  for (const dialog of [guestPinDialog, tableEntryDialog]) {
    assert.equal((dialog.match(/type="password"/g) || []).length, 2)
    assert.equal((dialog.match(/inputMode="numeric"/g) || []).length, 2)
    assert.equal((dialog.match(/pattern="\[0-9\]\*"/g) || []).length, 2)
    assert.equal((dialog.match(/onBeforeInput=\{blockNonNumericPinInput\}/g) || []).length, 2)
    assert.match(dialog, /onKeyDown=\{blockNonNumericPinKey\}/)
  }

  assert.equal((guestPinDialog.match(/maxLength=\{expectedPinLength\}/g) || []).length, 2)
  assert.equal((tableEntryDialog.match(/maxLength=\{GUEST_MODE_PIN_LENGTH\}/g) || []).length, 2)
  assert.match(guestMode, /GUEST_MODE_PIN_LENGTH = 2/)
  assert.match(guestMode, /pinLength: expectedPinLength/)
  assert.match(guestMode, /getGuestModePinLength\(session\)/)
  assert.match(source, /function blockNonNumericPinKey\(event\)/)
  assert.match(source, /event\.key\.length === 1 && !\/\^\[0-9\]\$\/\.test\(event\.key\)/)
  assert.match(source, /function blockNonNumericPinInput\(event\)/)
  assert.match(source, /typeof event\.data === 'string' && \/\\D\/\.test\(event\.data\)/)
})

test('tablet Guest mode uses customer visibility while staff ordering keeps waiter visibility', () => {
  const waiterOrder = readSource('src/pages/WaiterOrder.jsx')

  assert.match(waiterOrder, /isGuestTabletMode\s*\n\s*\? isCustomerMenuCategory\(category, visibilityNow\)\s*\n\s*: isWaiterMenuCategory\(category, visibilityNow\)/)
  assert.match(waiterOrder, /isGuestTabletMode\s*\n\s*\? isCustomerMenuItem\(item, visibilityNow\)\s*\n\s*: isWaiterMenuItem\(item, visibilityNow\)/)
  assert.match(waiterOrder, /const menuAudience = isGuestTabletMode \? 'public' : 'waiter'/)
  assert.match(waiterOrder, /audience=\{menuAudience\}/)
})

test('WaiterOrder keeps the table pricing choice out of the order header', () => {
  const source = readSource('src/pages/WaiterOrder.jsx')

  assert.doesNotMatch(source, /requestPriceModeChange|confirmPriceModeChange|pendingPriceMode/)
  assert.doesNotMatch(source, /getPriceModeLabel\(mode, lang\)/)
  assert.match(source, /setPriceMode\(resolveOrderingPriceMode\(activeOrder, state\.cart\)\)/)
  assert.match(source, /getMenuItemForPriceMode\(item, priceMode\)/)
})

test('table entry collects pricing and PIN before opening the locked customer menu', () => {
  const waiterTables = readSource('src/pages/WaiterTables.jsx')
  const handleTable = functionBody(waiterTables, 'handleTable')
  const openTableForGuest = functionBody(waiterTables, 'openTableForGuest')
  const seatReservation = functionBody(waiterTables, 'seatReservation')
  const handleManageOrder = functionBody(waiterTables, 'handleManageOrder')

  assert.match(handleTable, /getTableGuestEntryContext\(table\.id, state\.orders\)/)
  assert.match(handleTable, /setPendingGuestTable\(\{ table, entryContext \}\)/)
  assert.doesNotMatch(handleTable, /SET_TABLE|CLEAR_CART|navigate\(/)
  assert.match(openTableForGuest, /const session = await createGuestModeSession\([\s\S]*priceMode: selectedPriceMode/)
  assert.match(openTableForGuest, /writeGuestModeSession\(session\)[\s\S]*SET_TABLE[\s\S]*CLEAR_CART[\s\S]*navigate\(session\.routePath/)
  assert.match(openTableForGuest, /activeOrderIds: latestContext\.activeOrderIds/)
  assert.match(seatReservation, /handleTable\(table\)/)
  assert.doesNotMatch(seatReservation, /UPDATE_TABLE|clearReservationPatch/)
  assert.match(handleManageOrder, /navigate\(`\/waiter\/order\/\$\{table\.id\}\?panel=order`\)/)
})
...
test('AnimatedSearch provides reusable smooth expandable search controls', () => {
  ...
  assert.match(waiterOrder, /<AnimatedSearch[\s\S]*?alwaysOpen/)
  assert.match(source, /const isOpen = alwaysOpen \|\| open/)
  assert.match(source, /!alwaysOpen && \(\s*<button/)
})
