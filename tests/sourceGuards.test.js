...
test('menu items support required option variants with parent product ids', () => {
  ...
  assert.match(waiterOrder, /menuItemRequiresOptions\(item, 'waiter'\)/)
  assert.match(waiterOrder, /getMenuItemOptionGroups\(item, lang, \{ audience: 'waiter' \}\)\.length > 0/)
  assert.match(waiterOrder, /quantity: normalizeMenuQuantity\(qty, item\)/)
  assert.match(waiterOrder, /currentQty=\{getMenuItemOptionGroups\(detailItem, lang, \{ audience: 'waiter' \}\)\.length > 0 \? 0/)
  assert.match(waiterOrder, /map\[i\.menu_item_id\] = \(map\[i\.menu_item_id\] \|\| 0\) \+ \(Number\(i\.quantity\) \|\| 0\)/)
  assert.match(waiterOrder, /if \(menuItemRequiresOptions\(item, 'waiter'\) \|\| isMenuItemSoldByWeight\(item\)\) \{\s*openDetail\(item\)/)
  assert.match(functionBody(waiterOrder, 'handleDecrement'), /if \(menuItemRequiresOptions\(item, 'waiter'\) \|\| isMenuItemSoldByWeight\(item\)\) \{\s*openDetail\(item\)[\s\S]*return/)
  assert.match(kitchenCheckReceipt, /getManualOrderNotes\(item, menuItem, lang\)/)
