...
test('menu and category writes surface Supabase errors', () => {
  ...
  assert.match(db, /case 'ADD_MENU_ITEM': \{[\s\S]*const \{ error \} = await supabase\.from\('menu_items'\)\.insert\(fields\)[\s\S]*if \(error\) throw error/)
  assert.match(db, /case 'ADD_MENU_ITEM': \{[\s\S]*\.from\('menu_item_costs'\)\.upsert\([\s\S]*if \(costError\) throw costError/)
...
test('menu costs stay private, snapshot onto sold items, and drive profit outputs', () => {
  ...
  assert.match(telegramApi, /revenueTotal - getOrdersCostTotal\(orders, menuItemMap\)/)
  assert.match(telegramMessage, /Чистая прибыль · Заказ/)
  assert.match(telegramMessage, /Маржа прибыли · Заказ/)
  assert.match(telegramMessage, /Чистая прибыль · Сегодня/)
...
test('AdminMenu product editor formats price inputs while storing digits', () => {
  ...
  assert.match(pricingFields, /label=\{`\$\{labels\.cost\} \(UZS\)`\}[\s\S]*value=\{form\.cost_price\}[\s\S]*onChange=\{setF\('cost_price'\)\}/)
