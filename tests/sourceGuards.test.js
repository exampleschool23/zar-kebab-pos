...
test('WaiterOrder prints cook checks by submitted order round', () => {
  ...
  assert.match(panel, /getItemName\(menuItemMap\[item\.menu_item_id\], 'ru'\)/)
  assert.match(panel, /buildKitchenCheckHtml\(\{ group: russianGroup, lang: 'ru', restaurantName \}\)/)
...
test('owner order deletion is routed through an owner-only RPC and wired to cashier and reports', () => {
  ...
  assert.match(cashierTables, /const isOwner = profile\?\.role === 'owner' \|\| state\.user\?\.role === 'owner'/)
...
test('expenses page is owner-only, persisted, and included in owner reports net income', () => {
  const app = readSource('src/App.jsx')
  const sidebar = readSource('src/components/UnifiedSidebar.jsx')
  const permissions = readSource('src/lib/permissions.js')
  ...
  assert.match(app, /path="\/admin\/expenses"/)
  assert.match(app, /path="\/admin\/expenses\/salaries"/)
  assert.match(app, /PAGE_ACCESS\.expenses/)
  assert.match(sidebar, /key: 'expenses'/)
  assert.match(sidebar, /path: '\/admin\/expenses'/)
  ...
  assert.match(expenses, /navigate\('\/admin\/expenses\/salaries'\)/)
  ...
  assert.match(salaries, /profile_id: employee\?\.id \|\| null/)
  assert.match(salaries, /System account|Аккаунт в системе|Tizim akkaunti/)
