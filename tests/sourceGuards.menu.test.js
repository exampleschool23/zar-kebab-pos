...
test('hidden menu categories are hidden from customer-facing menus only', () => {
  ...
  assert.match(waiterOrder, /isWaiterMenuCategory\(category, visibilityNow, staffUserId, orderType\)/)
...
test('menu item deletion is soft-delete only so history analytics keep lookup context', () => {
  ...
  assert.match(itemCard, /touch-manipulation flex h-11 items-center/)
  assert.match(itemCard, /Archive item/)
  assert.match(itemCard, /<Archive size=/)
  ...
  assert.match(itemRow, /touch-manipulation flex h-11 items-center/)
  assert.match(itemRow, /Archive item/)
  assert.match(itemRow, /<Archive size=/)
